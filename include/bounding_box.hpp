...
struct BoundingBox {
    ...
    BoundingBox(const interval &x, const interval &y,const interval &z) : x(x.expand()), y(y.expand()), z(z.expand()) {
    ...
    bool hit(const Ray &r, interval t) const {
        for (u32 axis = 0; axis < 3; axis++) {
            ...
            if (t0 < t1) {
                if (t0 > t.start) t.start = t0;
                if (t1 < t.end) t.end = t1;
            } else {
                if (t1 > t.start) t.start = t1;
                if (t0 < t.end) t.end = t0;
            }
