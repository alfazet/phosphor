...
void SceneBuffers::upload_scene(ClContext &ctx, const SceneData &scene, const Bvh &bvh) {
    ...
    this->n_etriangles = scene.emissive_triangles.size();
    ...
    std::vector<float4> etv0(n_etriangles), etv1(n_etriangles), etv2(n_etriangles);
    std::vector<float2> etuv0(n_etriangles), etuv1(n_etriangles), etuv2(n_etriangles);
    std::vector<float4> etn0(n_etriangles), etn1(n_etriangles), etn2(n_etriangles);
    std::vector<float4> ett0(n_etriangles), ett1(n_etriangles), ett2(n_etriangles);
    std::vector<u32> etmat(n_etriangles);
    for (u32 i = 0; i < n_etriangles; i++) {
    ...
    this->etri_v0 = dev_buf(ctx, etv0.data(), n_etriangles, sizeof(float4));
    this->etri_v1 = dev_buf(ctx, etv1.data(), n_etriangles, sizeof(float4));
    this->etri_v2 = dev_buf(ctx, etv2.data(), n_etriangles, sizeof(float4));
    this->etri_uv0 = dev_buf(ctx, etuv0.data(), n_etriangles, sizeof(float2));
    this->etri_uv1 = dev_buf(ctx, etuv1.data(), n_etriangles, sizeof(float2));
    this->etri_uv2 = dev_buf(ctx, etuv2.data(), n_etriangles, sizeof(float2));
    this->etri_n0 = dev_buf(ctx, etn0.data(), n_etriangles, sizeof(float4));
    this->etri_n1 = dev_buf(ctx, etn1.data(), n_etriangles, sizeof(float4));
    this->etri_n2 = dev_buf(ctx, etn2.data(), n_etriangles, sizeof(float4));
    this->etri_mat_index = dev_buf(ctx, etmat.data(), n_etriangles, sizeof(u32));
...
void SceneBuffers::set_camera(const CameraParams &cam, u32 width, u32 height, u32 iters) {
    this->camera = cam;
